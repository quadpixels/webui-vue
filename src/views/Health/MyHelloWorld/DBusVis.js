class DBusVis {
  static Hey() {
    console.log(document.getElementById('my_canvas_dbus'));
    OnMount();
  }
}

var dbus_timeline_view;
...
const TEXT_Y0 = 3;
const HISTOGRAM_W = 100,
  HISTOGRAM_H = LINE_SPACING;
const HISTOGRAM_X = 270;
const HISTOGRAM_LEFT_TAIL_WIDTH = -1,
  HISTOGRAM_RIGHT_TAIL_WIDTH = -1;
...
function RenderHistogramForImageData(ctx, key) {
  ...
  for (let i = 0; i < buckets.length; i++) {
    ...
    if (buckets[i] > 0) {
      ...
      // Write thresholds
      if (
        cumDensity0 < HISTOGRAM_LEFT_TAIL_WIDTH &&
      ...
      if (
        cumDensity0 < 1 - HISTOGRAM_RIGHT_TAIL_WIDTH &&
...
class TimelineView {
  ...
  BeginWarpToRequestAnimation(req) {
  ...
  UpdateAnimation() {
    const EPS = 1e-3;
    if (
      Math.abs(this.LowerBoundTime - this.LowerBoundTimeTarget) < EPS &&
  ...
  RenderHistogram(ctx, key, xMid, yMid) {
    ...
    let hist = GetHistoryHistogram()[key];
    ctx.putImageData(
      this.IpmiVizHistogramImageData[key],
      xMid - HISTOGRAM_W / 2,
      yMid - HISTOGRAM_H / 2
    );
  ...
  MouseXToTimestamp(x) {
    let ret =
      ((x - LEFT_MARGIN) / (RIGHT_MARGIN - LEFT_MARGIN)) *
  ...
  OnMouseMove() {
    ...
    let lineIndex = Math.floor(
      (this.MouseState.y - YBEGIN + TEXT_Y0) / LINE_SPACING
    );
    ...
    if (lineIndex < this.Intervals.length) {
      ...
      if (
        this.MouseState.x <= PAD + LINE_SPACING / 2 + LEFT_MARGIN &&
  ...
  OnMouseDown() {
    ...
    if (
      this.MouseState.hoveredLineIndex >= 0 &&
    ...
    if (tx >= t0 && tx <= t1) {
      ...
      // Otherwise start a new dragging session
  ...
  Render(ctx) {
    ...
    if (this.IsCanvasDirty) {
      ...
      if (
        ...
        let dy =
          YBEGIN +
          LINE_SPACING * MouseState.hoveredLineIndex -
          LINE_SPACING / 2;
      ...
      if (bidx < breakWidths.length) {
        ...
        while (bidx < breakWidths.length) {
          ...
          if (
            t2 + breakWidths[bidx] >= this.LowerBoundTime &&
      ...
      let title_start_idx = this.TitleStartIdx,
        title_end_idx = title_start_idx;
      ...
      for (let j = this.TitleStartIdx; j < this.Intervals.length; j++) {
          ...
          for (let i = 0; i < intervals_j.length; i++) {
            ...
            if (
              ...
              if (GetHistoryHistogram()[this.Titles[j]] != undefined) {
                ...
                let bucketInterval =
                  (histogramEntry[1] - histogramEntry[0]) /
        ...
        // Triangle markers for entries outside of the viewport
        {
          const PAD = 2,
            H = LINE_SPACING;
          if (
            this.MouseState.hoveredLineIndex == j &&
            this.MouseState.hoveredSide == 'left'
          ) {
      ...
      if (this.IsTimeDistributionEnabled) {
        ...
        for (let j = this.TitleStartIdx; j < this.Intervals.length; j++) {
          if (this.IpmiVizHistHighlighted[this.Titles[j]] != undefined) {
            let entry = HistogramThresholds[this.Titles[j]];
            const theSet = Array.from(
              this.IpmiVizHistHighlighted[this.Titles[j]]
            );
      ...
      if (
        ...
        if (this.IsHighlighted() == false) {
          let label =
            '' +
            this.MouseXToTimestamp(this.MouseState.x).toFixed(
              toFixedPrecision
            ) +
            's';
          ctx.fillStyle = '#0000FF';
...
class DBusTimelineView extends TimelineView {
  RenderToolTip(
    ...
    let w = 1,
      h = LINE_SPACING * labels.length + 2 * PAD;
...
function Preprocess_DBusPcap(data, timestamps) {
  console.log('Preprocess_DBusPcap is not used in the WebUI-Vue version');
  console.log(data);
...
function OnMount() {
  dbus_timeline_view = new DBusTimelineView();
  ...
  for (let i = 0; i < 7; i++) {
    document
      .getElementById(tags[i])
      .addEventListener('click', OnGroupByConditionChanged_DBus);
  }
  g_renderer = new Renderer();
  console.log('g_renderer=' + g_renderer);

  // Input
  Canvas_DBus = document.getElementById('my_canvas_dbus');
